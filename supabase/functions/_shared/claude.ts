...
export const buildContextPrompt = (
  ...
  const favoriteRecipes = recipes.filter((r: any) => r.is_favorite);
  ...
  if (favoriteRecipes.length > 0) {
    ...
    favoriteRecipes.slice(0, 5).forEach((recipe: any) => {
  ...
  if (pantryItems.length > 0) {
    ...
    pantryItems.slice(0, 15).forEach((item: any) => {
  ...
  if (upcomingMeals.length > 0) {
    ...
    upcomingMeals.slice(0, 10).forEach((meal: any) => {
  ...
  if (groceryItems.length > 0) {
    const unchecked = groceryItems.filter((item: any) => !item.is_checked);
    if (unchecked.length > 0) {
      context += `Items on grocery list (need to buy):\n`;
      unchecked.slice(0, 15).forEach((item: any) => {
        context += `- ${item.name} (${item.quantity} ${item.unit})\n`;
      });
      context += `\n`;
    }
